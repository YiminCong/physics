...
**Demonstration.** The BCS density of states is N_s(E) = N(0)·E/√(E² − Δ²) for E > Δ and **zero inside the gap**. So the tunneling I–V shows **no current until eV exceeds Δ**, then a sharp rise — a direct readout of the gap (computed via the golden rule and DOS of Modules 3.8 and 4.5).

**演示。** BCS 态密度为 N_s(E) = N(0)·E/√(E² − Δ²)（E > Δ 时），**能隙内为零**。因此隧穿 I–V 曲线显示**直到 eV 超过 Δ 时才有电流**，随后急剧上升——这是对能隙的直接读出（通过模块 3.8 和 4.5 的黄金法则与态密度计算得出）。
