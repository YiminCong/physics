...
- **DC Josephson effect:** a supercurrent I = I_c sin(Δφ) flows at **zero voltage**, set by the phase difference Δφ across the junction.
...
- **直流约瑟夫森效应：** 超电流 I = I_c sin(Δφ) 在**零电压**下流动，由结两端的相位差 Δφ 决定。
- **交流约瑟夫森效应：** 恒定电压 V 产生频率为 f = 2eV/h 的**振荡**电流。

**Demonstration.** The controlling variable is the **phase φ** of the order parameter (Module 0.4) — superconductivity makes a quantum phase macroscopically observable. A **SQUID** (two junctions in a loop) turns phase interference into the most sensitive magnetometer known.

**演示。** 控制变量是序参量的**相位 φ**（模块 0.4）——超导性使量子相位在宏观上可观测。**SQUID**（回路中的两个结）将相位干涉转变为已知最灵敏的磁强计。
