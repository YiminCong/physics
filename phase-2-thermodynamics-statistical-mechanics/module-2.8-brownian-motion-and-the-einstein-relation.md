...
**定义。** **布朗运动**是悬浮在流体中的微观粒子（花粉粒、胶体粒子）由于与流体分子的非平衡碰撞而产生的持续随机抖动。**爱因斯坦（1905）**将其建模为随机游走，并证明**均方位移随时间线性增长**：⟨x²⟩ = 2Dt（一维），其中 D 为**扩散系数**。粒子平均而言不发生漂移，但以扩散方式扩展。
...
**演示。** 爱因斯坦通过**爱因斯坦关系** D = μ k_B T 将这种微观随机性与可测量的宏观系数联系起来，其中 μ 为粒子的迁移率（单位力下的漂移速度）。对于粘度为 η 的流体中半径为 r 的球体，斯托克斯定律给出 μ = 1/(6πηr)，故 **D = k_B T / (6πηr)**（斯托克斯–爱因斯坦关系）。因此测量 D 和 η 可得到**阿伏加德罗常数**——这正是**佩兰实验（1908）**所做的，为原子的真实存在提供了决定性证明。
...
**定义。** 爱因斯坦关系 D = μ k_B T 是**涨落–耗散定理**的第一个实例：系统的随机涨落（扩散 D）与其耗散响应（迁移率/摩擦 μ）相互联系，两者均源于相同的分子碰撞。
...
1. 写出爱因斯坦结果 ⟨x²⟩ = 2Dt，并将扩散式扩展与弹道运动进行对比。
2. 陈述爱因斯坦关系 D = μ k_B T 和斯托克斯–爱因斯坦形式；解释它如何给出阿伏加德罗常数。
