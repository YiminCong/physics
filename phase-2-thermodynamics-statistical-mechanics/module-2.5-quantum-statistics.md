...
- **费米子**（半整数自旋）：波函数反对称。没有两个费米子可以占据同一单粒子态——**泡利不相容原理**。能量为 ε 的态的平均占据数为**费米–狄拉克分布**：
...
两种情况下，μ 均为**化学势**，由条件 Σ n(ε) = N（总粒子数）确定。在高温或低密度下（对所有占据态有 e^{(ε − μ)/k_BT} ≫ 1），两种分布均退化为经典麦克斯韦–玻尔兹曼结果 n ∝ e^{−ε/k_BT}。
...
**演示。** 这些分布在巨正则系综（模块 2.4）中推导。对于费米子，每个单粒子态 α 是一个可容纳 0 或 1 个粒子的独立子系统；其巨正则配分函数为
...
平均占据数为 ⟨n_α⟩ = −∂(k_BT ln z_α)/∂ε_α = n_FD(ε_α)。对于玻色子，该态可容纳 0、1、2、…个粒子；等比级数给出 z_α = 1/(1 − e^{−(ε_α − μ)/k_BT})，⟨n_α⟩ = n_BE(ε_α)。分母符号之差——费米子为加，玻色子为减——编码了两种统计之间的全部区别。
...
这定义了动量空间中的费米面——电子结构、电导率和超导性（第 4–5 阶段）的核心对象。对于玻色子，μ 必须保持低于基态能量；当温度降低使 μ → 0 时，基态的宏观占据（玻色–爱因斯坦凝聚）发生——见模块 2.6。
...
**定义。** 化学势 μ 是在固定 T 和 V 下添加一个粒子的自由能代价：μ = (∂F/∂N)_{T,V}。对于费米子，T = 0 时 μ = E_F。在有限温度下，μ 略微低于 E_F（对于金属，偏移量约为 (k_BT)²/E_F——在室温下很小，因为 k_BT ≈ 0.025 eV 而 E_F ≈ 5–10 eV）。对于三维玻色子，在所有高于凝聚温度 T_BEC 的温度下均有 μ < ε_0（基态能量）。
...
它远小于经典能均分值 (3/2)N k_B，因为只有费米面附近约 ∼ k_BT/E_F 比例的电子是热活跃的。这个与 T 成正比的热容是费米统计的直接特征，在金属中可测量，并在模块 4.1 中进一步讨论。
...
- **声子和光子**：两者均为玻色子（整数自旋）。它们的化学势为零（不守恒），故 n_BE(ε) = 1/(e^{ε/k_BT} − 1) 直接给出光子的普朗克分布和声子模式的玻色占据——支配黑体辐射和晶格热容（模块 2.6、模块 4.3）。
...
2. 用一句话解释为什么电子热容 C_el ∝ T 而非经典值 (3/2)N k_B。哪些电子有贡献？
3. 对于 T = 0 的自由费米气体，利用所有低于 E_F 的态均被填满的条件，推导用数密度 n 表示的费米能 E_F。
4. 为什么玻色子的化学势必须满足 μ < ε_0（基态能量）？当 μ → ε_0 时会发生什么？
