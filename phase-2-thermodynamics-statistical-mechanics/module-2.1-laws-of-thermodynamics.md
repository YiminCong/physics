...
其中 δQ 和 δW 是路径依赖的（非恰当微分）。对于气体，δW = P dV。**热力学第二定律**指出，孤立系统的熵 S 永不减少；对于可逆过程，dS = δQ/T；对于不可逆过程，dS > δQ/T。**热力学第三定律**（能斯特定理）指出，对于完美晶体，当 T → 0 时 S → 0，使得绝对熵有明确定义。
...
**应用。** 不等式 η ≤ 1 − T_C/T_H 是工程上的绝对上限：一座燃烧温度为 800 K、向 300 K 散热的发电厂无论机械设计多么精妙，效率都不能超过 62.5%。更深层地，第二定律赋予时间一个箭头——自发过程增加总熵，从而区分过去与未来。第一定律阻止第一类永动机（凭空创造能量）；第二定律阻止第二类永动机（在循环中将热量全部转化为功）。
...
**定义。** 对于将系统从状态 A 带到状态 B 的任意过程，克劳修斯不等式给出 ∮ δQ/T ≤ 0（沿循环），仅在可逆循环时取等号。态函数熵通过可逆路径定义：
...
**演示。** 对于理想气体向真空的自由膨胀（不可逆过程），δQ = 0 且 δW = 0，故 ΔU = 0，温度不变。然而体积翻倍，每个分子的熵增加 ΔS = n k_B ln 2——这是通过假想一个连接相同初末态的可逆等温膨胀来计算的。即便是气体自身完成了所有增加无序度的"工作"，宇宙的熵仍然升高了。
...
**应用。** 熵是宏观热力学与微观世界之间的纽带。玻尔兹曼关系 S = k_B ln Ω（其中 Ω 为可及微观态数目）将在模块 2.4 中从统计力学推导出来，并支撑模块 2.5 中的量子统计。
...
2. 仅利用循环可逆（ΔS_universe = 0）的条件，推导卡诺效率 η = 1 − T_C/T_H。
3. 一台冰箱将热量从 T_C = 250 K 传输到 T_H = 300 K。从冷源取出每焦耳热量所需的最小功是多少？
4. 为什么 dU 是恰当微分，而 δQ 和 δW 单独来看不是？给出一个具体例子说明 δQ 的路径依赖性。
