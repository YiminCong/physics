...
- Schwarzschild $ds^2 = -(1 - r_s/r)c^2dt^2 + (1 - r_s/r)^{-1}dr^2 + r^2 d\Omega^2$, $r_s = 2GM/c^2$ · 史瓦西解与视界
- $\Box\,\bar{h}_{\mu\nu} = -\dfrac{16\pi G}{c^4}T_{\mu\nu}$ — linearized gravity, wave equation · 线性化引力波动方程
- Two transverse polarizations $h_+, h_\times$ travel at $c$ · 两个横向偏振以光速传播
- Quadrupole power $P = \dfrac{G}{5c^5}\langle\dddot{Q}_{ij}\dddot{Q}^{ij}\rangle$ — gravitational radiation · 四极辐射功率
...
**1.** Setting the escape speed to $c$: $\tfrac12 c^2=GM/r\Rightarrow r_s=2GM/c^2$ (and $[GM/c^2]=$ length). At $r_s$ the curvature invariants (e.g. $R_{\mu\nu\rho\sigma}R^{\mu\nu\rho\sigma}$) stay finite, so it is a **coordinate** singularity (removable by Kruskal coordinates); only $r=0$ is a true physical singularity. · $r_s=2GM/c^2$;曲率有限故为坐标奇点,$r=0$ 才是物理奇点。

**2.** Strain is the fractional change: $\Delta L/L=h=5\times10^{-22}$, so $\Delta L\approx h L=2\times10^{-18}$ m over $L=4$ km. LIGO's Fabry–Pérot cavity reflects the light $\sim10^2$–$10^3$ times, multiplying the effective arm length and the accumulated phase shift. · $\Delta L/L=h$,$\Delta L\approx2\times10^{-18}$ m;法布里–珀罗腔放大有效臂长。

**3.** Two polarizations $h_+$ and $h_\times$. A ring of test particles under $h_+$ stretches along $x$ while squeezing along $y$, then reverses; under $h_\times$ the same occurs along axes rotated by $45^\circ$. Over one cycle the ring oscillates between orthogonal ellipses. · 两偏振 $h_+,h_\times$;测试粒子环在椭圆间振荡(相差 $45^\circ$)。
