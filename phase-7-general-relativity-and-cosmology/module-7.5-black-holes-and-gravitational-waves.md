...
**定义。** 静态球对称时空的真空爱因斯坦方程（R_{μν} = 0，模块 7.4）最简单的精确解是*史瓦西度规*（1916 年）：
...
其中 dΩ² = dθ² + sin²θ dφ² 是单位二维球面的度规，*史瓦西半径*为 r_s = 2GM/c²。在 r = r_s 处，系数 g₀₀ 为零而 g_{rr} 发散——但这是*坐标奇点*：在克鲁斯卡尔-塞凯赖什坐标下度规在此处光滑。在 r = 0 处，曲率不变量 Rμνρσ R^{μνρσ} 发散——这是真正的*物理奇点*，标志着经典广义相对论的失效。
...
曲面 r = r_s 是*事件视界*：一个零超曲面，任何因果信号（包括光）都无法从中逃逸至 r → ∞。自由穿越视界的观察者在局域上感受不到任何异常（根据等效原理；模块 7.1），而远处的观察者则看到下落观察者的信号红移并在视界处"冻结"。
...
**演示。** 太阳的史瓦西半径：r_s = 2GM_☉/c² ≈ 2 × (6.67 × 10⁻¹¹)(2 × 10³⁰)/(9 × 10¹⁶) ≈ 2.95 km。太阳必须被压缩到 3 km 以下的半径才能形成黑洞。对于恒星级黑洞（M ≈ 10 M_☉），r_s ≈ 30 km；对于 M87 中心的超大质量黑洞（M ≈ 6.5 × 10⁹ M_☉），r_s ≈ 190 亿 km ≈ 19 AU。
...
将测地线方程（模块 7.3）应用于史瓦西度规，得到最内稳定圆轨道（ISCO）在 r = 3r_s = 6GM/c² 处——在此半径以内不存在稳定圆轨道，物质向内螺旋。光子轨道在 r = (3/2)r_s 处（光子球）。这些结果纯粹来自度规，无需额外物理。
...
- *恒星级*（5–100 M_☉）：大质量恒星坍缩的终态，通过 X 射线双星和引力波探测。
- *超大质量*（10⁶–10¹⁰ M_☉）：位于星系核心；由事件视界望远镜直接成像（M87*，2019 年；Sgr A*，2022 年）。
- 霍金辐射（半经典广义相对论，此处不作详述）预言黑洞以温度 T_H = ℏc³/(8πGMk_B) 缓慢蒸发，仅对原初或微观黑洞有意义。
...
**定义。** 在远离源或弱场情况下，令 g_{μν} = ημν + hμν，其中 |hμν| ≪ 1。将 EFE 对 hμν 展开至一阶，并选取*洛伦兹规范*（∂^μ h̄_{μν} = 0，其中 h̄_{μν} = hμν − ½ ημν h 为迹反转微扰），得：
...
其中 □ = −(1/c²)∂²_t + ∇² 是平坦时空的达朗贝尔算符。在真空（Tμν = 0）中，这是一个波动方程：引力扰动以速度 c 传播。这就是*引力波*（GW）。
...
+ 偏振在 x 方向拉伸时空、在 y 方向压缩（反之亦然）；× 偏振在 45° 方向做同样的事。应变 h = ΔL/L 测量自由下落检验质量之间固有距离的分数变化——这是可观测量。
...
对于两个等质量 M 在间距 2r、轨道频率 Ω 的圆轨道上运行：P = (32G⁴M⁵)/(5c⁵r⁵)。随着能量辐射，轨道衰减——即*旋近*。赫尔斯-泰勒双脉冲星（1974 年）的轨道衰减与四极矩预测吻合至约 0.1%，在直接探测前 40 年提供了引力波的间接证据。
...
LIGO 的首次探测（GW150914，2015 年 9 月）观测到来自双黑洞并合（约 36 M_☉ + 29 M_☉，距离约 410 Mpc）的 h ∼ 10⁻²¹。该应变意味着在 4 km 基线上 ΔL ∼ 10⁻¹⁸ m——质子半径的千分之一——由激光干涉仪探测。
...
- *参数估计*：引力波信号编码了啁啾质量 M_c = (M₁M₂)^{3/5}/(M₁+M₂)^{1/5}、光度距离和自旋参数——允许对致密天体进行总体统计研究。
...
1. 利用量纲分析和逃逸速度等于 c 的条件推导史瓦西半径 r_s = 2GM/c²。为何 r_s 处是坐标奇点而非物理奇点？
2. 一列应变为 h = 5 × 10⁻²² 的引力波经过地球。间距 L = 4 km 的两个自由下落反射镜之间距离的分数变化是多少？LIGO 的法布里-珀罗腔如何提升灵敏度？
