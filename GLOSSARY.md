...
> Tip: use your browser/editor find (Ctrl/Cmd-F) to jump to any term in either language.

## Contents

- [Phase 0: Mathematical Foundations](#phase-0-mathematical-foundations)
- [Phase 1: Classical Physics](#phase-1-classical-physics)
- [Phase 2: Thermodynamics and Statistical Mechanics](#phase-2-thermodynamics-and-statistical-mechanics)
- [Phase 3: Quantum Mechanics](#phase-3-quantum-mechanics)
- [Phase 4: Condensed Matter](#phase-4-condensed-matter)
- [Phase 5: Superconductivity](#phase-5-superconductivity)
- [Phase 6: Quantum Field Theory and Many-Body Physics](#phase-6-quantum-field-theory-and-many-body-physics)
- [Phase 7: General Relativity and Cosmology](#phase-7-general-relativity-and-cosmology)
- [Phase 8: Particle Physics and the Standard Model](#phase-8-particle-physics-and-the-standard-model)
...
| Term | 中文 | Definition |
|------|------|------------|
| Newton's laws | 牛顿定律 | The three laws governing force and motion. |
| Inertial frame | 惯性系 | A non-accelerating reference frame. |
...
| Noether's theorem | 诺特定理 | Every continuous symmetry yields a conservation law. |
| Central force | 有心力 | A force directed along the line to a center. |
| Reduced mass | 约化质量 | The effective mass of a two-body relative motion. |
| Effective potential | 有效势 | Radial potential plus the centrifugal barrier. |
| Kepler's laws | 开普勒定律 | The three laws of planetary orbits. |
| Simple harmonic motion | 简谐运动 | Sinusoidal motion of a linear restoring force. |
| Resonance | 共振 | Large response when driving matches a natural frequency. |
| Normal modes | 简正模 | Collective oscillation patterns of a coupled system. |
| Moment of inertia tensor | 转动惯量张量 | The tensor relating angular velocity to angular momentum. |
| Euler's equations (rigid body) | 欧拉方程（刚体） | The rotational equations of motion for a rigid body. |
...
| Faraday's law | 法拉第定律 | A changing magnetic flux induces an electric field. |
| Displacement current | 位移电流 | Maxwell's term making the equations consistent. |
| Maxwell's equations | 麦克斯韦方程组 | The four equations of classical electromagnetism. |
| Poynting vector | 坡印廷矢量 | The directional energy flux of an EM field. |
| Electromagnetic wave | 电磁波 | A self-propagating oscillation of E and B fields. |
| Polarization | 偏振 | The orientation of a wave's oscillation. |
| Dipole radiation | 偶极辐射 | Radiation from an oscillating dipole. |
| Huygens' principle | 惠更斯原理 | Each wavefront point acts as a source of wavelets. |
| Interference / diffraction | 干涉 / 衍射 | Superposition effects and bending of waves. |
| Special relativity | 狭义相对论 | The theory of physics in inertial frames at high speed. |
| Lorentz transformation | 洛伦兹变换 | The coordinate transformation between inertial frames. |
| Time dilation | 时间膨胀 | Moving clocks run slow. |
| Length contraction | 长度收缩 | Moving objects are shortened along their motion. |
| Minkowski spacetime | 闵可夫斯基时空 | Flat 4D spacetime with the metric of special relativity. |
| Invariant interval | 不变间隔 | The frame-independent spacetime separation. |
| Four-vector | 四矢量 | A 4-component object transforming under Lorentz boosts. |
...
| Standing wave | 驻波 | A wave pattern with fixed nodes from superposition. |
| Doppler effect | 多普勒效应 | Frequency shift due to relative motion. |
| Group velocity | 群速度 | The speed at which a wave packet's envelope travels. |
| Continuity equation | 连续性方程 | Local statement of conservation (mass, charge, probability). |
| Euler equation (fluids) | 欧拉方程（流体） | Newton's law for an inviscid fluid. |
| Bernoulli's theorem | 伯努利定理 | Faster flow means lower pressure along a streamline. |
| Navier–Stokes equation | 纳维–斯托克斯方程 | The master equation of viscous fluid flow. |
| Reynolds number | 雷诺数 | The ratio of inertial to viscous forces. |
| Viscosity | 黏度 | A fluid's resistance to shear. |
| Vorticity | 涡量 | The local rotation of a fluid, curl of velocity. |
| Turbulence | 湍流 | Chaotic, eddying high-Reynolds flow. |
| Stress tensor | 应力张量 | Internal force per unit area in a continuum. |
| Strain tensor | 应变张量 | The local deformation of a continuum. |
| Hooke's law | 胡克定律 | Stress is proportional to strain for small deformation. |
| Young's modulus | 杨氏模量 | The stiffness relating tensile stress and strain. |
| Elastic wave (P / S) | 弹性波（纵波 / 横波） | Compressional and shear waves in a solid. |
| Nonlinear dynamics | 非线性动力学 | The study of nonlinear evolution equations. |
| Fixed point | 不动点 | An equilibrium of a dynamical system. |
| Bifurcation | 分岔 | A qualitative change in behavior as a parameter varies. |
| Chaos | 混沌 | Deterministic, aperiodic motion sensitive to initial conditions. |
| Lyapunov exponent | 李雅普诺夫指数 | The exponential rate at which nearby trajectories diverge. |
| Strange attractor | 奇怪吸引子 | A fractal attracting set of a chaotic system. |
| Logistic map | 逻辑斯蒂映射 | A simple iteration showing the period-doubling route to chaos. |

---

## Phase 2: Thermodynamics and Statistical Mechanics

| Term | 中文 | Definition |
|------|------|------------|
| Laws of thermodynamics | 热力学定律 | The zeroth–third laws governing heat, work, and entropy. |
| Temperature | 温度 | The quantity equalized at thermal equilibrium. |
...
| Order parameter | 序参量 | A quantity that is nonzero in an ordered phase. |
| Landau theory | 朗道理论 | Expansion of free energy in powers of the order parameter. |
...
| Fermi energy | 费米能 | The highest filled energy level at zero temperature. |
...
| Pauli exclusion principle | 泡利不相容原理 | No two fermions occupy the same state. |
| Slater determinant | 斯莱特行列式 | An antisymmetrized many-fermion wave function. |
| Perturbation theory | 微扰论 | Approximating corrections from a small added term. |
| Fine structure | 精细结构 | Relativistic and spin–orbit corrections to spectra. |
| Zeeman effect | 塞曼效应 | Splitting of levels in a magnetic field. |
| Variational principle | 变分原理 | Trial states give an upper bound on the ground energy. |
| WKB approximation | WKB 近似 | A semiclassical method for slowly varying potentials. |
...
| Time reversal | 时间反演 | Behavior under reversing the direction of time. |
| Second quantization | 二次量子化 | Formalism tracking occupation numbers via field operators. |
| Creation / annihilation operators | 产生 / 湮灭算符 | Operators that add / remove a particle in a mode. |
...
| Term | 中文 | Definition |
|------|------|------------|
| Drude model | 德鲁德模型 | Classical electron-gas model of conduction. |
| Sommerfeld model | 索末菲模型 | Quantum free-Fermi-gas model of metals. |
| Free electron gas | 自由电子气 | Non-interacting electrons filling states up to the Fermi level. |
| Electronic heat capacity | 电子比热 | The electron contribution, linear in temperature. |
| Bravais lattice | 布拉伐格子 | The periodic array of points defining a crystal. |
...
| Vortex | 涡旋 | A quantized flux line in a type-II superconductor. |
| Abrikosov lattice | 阿布里科索夫格子 | The triangular array of vortices in the mixed state. |
| Flux pinning | 磁通钉扎 | Trapping of vortices on defects, enabling high currents. |
...
## Phase 6: Quantum Field Theory and Many-Body Physics

| Term | 中文 | Definition |
|------|------|------------|
| Second quantization | 二次量子化 | The occupation-number formalism for many particles. |
| Field operator | 场算符 | An operator that creates/annihilates a particle at a point. |
| Fock space | 福克空间 | The many-body state space of variable particle number. |
| Green's function | 格林函数 | The amplitude to add and later remove a particle. |
| Propagator | 传播子 | The Green's function viewed as particle propagation. |
| Spectral function | 谱函数 | The distribution of excitation energies from the Green's function. |
| Quasiparticle | 准粒子 | A dressed, weakly interacting effective excitation. |
| Kramers–Kronig relations | 克拉默斯–克勒尼希关系 | Causality links between real and imaginary response parts. |
| Feynman diagram | 费曼图 | A pictorial term in a perturbation expansion. |
| Wick's theorem | 威克定理 | Reduces products of operators to sums of pairings. |
| Self-energy | 自能 | The correction to a particle's propagation from interactions. |
| Dyson equation | 戴森方程 | Resums self-energy into the full propagator. |
| Random phase approximation (RPA) | 无规相位近似 | A standard resummation describing screening. |
| Path integral | 路径积分 | The sum-over-histories formulation of quantum theory. |
...
| Term | 中文 | Definition |
|------|------|------------|
| Equivalence principle | 等效原理 | Free fall is locally indistinguishable from no gravity. |
| Curved spacetime | 弯曲时空 | Gravity represented as the curvature of spacetime. |
| Metric tensor | 度规张量 | The field g defining spacetime distances. |
| Christoffel symbols | 克里斯托费尔符号 | Connection coefficients for covariant differentiation. |
| Covariant derivative | 协变导数 | Differentiation that respects curvature. |
| Riemann curvature tensor | 黎曼曲率张量 | The full measure of spacetime curvature. |
| Ricci tensor / scalar | 里奇张量 / 标量 | Contractions of the Riemann tensor. |
...
| FLRW metric | FLRW 度规 | The homogeneous, isotropic cosmological metric. |
| Friedmann equations | 弗里德曼方程 | The evolution equations of the cosmic scale factor. |
| Hubble's law | 哈勃定律 | Recession velocity proportional to distance. |
| Big Bang | 大爆炸 | The hot, dense early state of the expanding universe. |
...
## Phase 8: Particle Physics and the Standard Model

| Term | 中文 | Definition |
|------|------|------------|
| Gauge symmetry | 规范对称性 | Invariance under local internal transformations. |
| Global / local symmetry | 整体 / 局域对称性 | Same transformation everywhere vs. point-dependent. |
...
| Higgs mechanism | 希格斯机制 | Mass generation via a symmetry-breaking field; the vacuum analog of superconductivity. |
| Higgs boson | 希格斯玻色子 | The excitation of the Higgs field, found in 2012. |
| Standard Model | 标准模型 | The current theory of particles and three forces. |
| Generations | 代 | The three repeating families of quarks and leptons. |
| Neutrino oscillation | 中微子振荡 | Flavor change of neutrinos, implying neutrino mass. |
| CP violation | CP 破坏 | Asymmetry between matter and antimatter behavior. |
| Supersymmetry | 超对称 | A proposed symmetry pairing bosons and fermions. |
| Grand unification | 大统一 | The idea that the forces merge at high energy. |
