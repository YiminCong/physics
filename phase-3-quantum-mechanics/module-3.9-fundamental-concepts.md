...
**Definition.** Build QM from kets |α⟩, bras ⟨α|, and operators acting on an abstract state space — *before* choosing a representation. Measurement projects onto eigenkets; the position and momentum "wavefunctions" are just the components ⟨x|ψ⟩ and ⟨p|ψ⟩ in particular bases.

**定义。** 从右矢 |α⟩、左矢 ⟨α| 以及作用于抽象态空间的算符出发构建量子力学——*在*选择表象*之前*。测量将态投影到本征右矢；位置和动量"波函数"只是在特定基下的分量 ⟨x|ψ⟩ 和 ⟨p|ψ⟩。

**Demonstration.** The Stern–Gerlach / spin-½ system is the cleanest arena: base kets |+⟩, |−⟩, operators as 2×2 matrices, and sequential measurements illustrate incompatibility directly.

**演示。** 斯特恩–盖拉赫 / 自旋-½ 系统是最简洁的舞台：基右矢 |+⟩、|−⟩，算符以 2×2 矩阵表示，序列测量直接展示不相容性。
