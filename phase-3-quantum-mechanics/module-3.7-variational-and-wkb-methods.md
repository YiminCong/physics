...
**Definition.** For *any* trial state ψ_trial, ⟨Ĥ⟩ ≥ E_ground. Minimizing ⟨Ĥ⟩ over adjustable parameters gives an upper-bound estimate of the ground-state energy.

**定义。** 对于*任意*试探态 ψ_trial，均有 ⟨Ĥ⟩ ≥ E_ground。对可调参数最小化 ⟨Ĥ⟩ 给出基态能量的上界估计。
...
**Definition.** For slowly varying potentials, ψ ≈ exp[(i/ℏ)∫p(x)dx], a semiclassical form. It gives quantization conditions and **tunneling rates**: transmission ∝ exp[−(2/ℏ)∫|p|dx] across a barrier.

**定义。** 对于缓慢变化的势，ψ ≈ exp[(i/ℏ)∫p(x)dx]，这是一种半经典形式。它给出量子化条件和**隧穿率**：穿越势垒的透射率 ∝ exp[−(2/ℏ)∫|p|dx]。
...
**应用。** 变分法是量子化学和试探态方法的基础（包括 BCS 变分基态，第 5 阶段）。WKB 隧穿解释了 α 衰变、扫描隧道显微镜，以及探测超导能隙的隧道效应（第 5 阶段）。
