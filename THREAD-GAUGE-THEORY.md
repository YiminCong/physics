...
| Classical EM · 经典电磁 | U(1), *global* phase | electromagnetic A_μ | ∂_μF^{μν} = μ₀J^ν | [1.10](./phase-1-classical-physics/module-1.10-electrodynamics-maxwell-equations.md), [1.15](./phase-1-classical-physics/module-1.15-covariant-electromagnetism.md) |
| Quantum EM · 量子电磁 | U(1) *local* phase | photon (gauged) | D_μ = ∂_μ + ieA_μ | [8.2](./phase-8-particle-physics-and-standard-model/module-8.2-quantum-electrodynamics.md) |
| The gauge principle · 规范原理 | local U(1) → SU(N) | Yang–Mills A^a_μ | F^a_{μν} = ∂_μA^a_ν − ∂_νA^a_μ + gf^{abc}A^b_μA^c_ν | [8.1](./phase-8-particle-physics-and-standard-model/module-8.1-symmetries-and-gauge-theory.md) |
| Strong force · 强相互作用 | SU(3)_color | 8 gluons (self-interacting) | β(g) < 0 → asymptotic freedom | [8.3](./phase-8-particle-physics-and-standard-model/module-8.3-quantum-chromodynamics.md) |
| Broken gauge symmetry · 破缺规范对称 | SU(2)_L × U(1)_Y → U(1)_em | W±, Z⁰ get mass; photon massless | m_W = ½gv, m_Z = m_W/cosθ_W | [8.4](./phase-8-particle-physics-and-standard-model/module-8.4-electroweak-theory-and-higgs.md) |
| Condensed-matter sibling · 凝聚态对应 | local U(1) (Cooper pairs) | photon gets mass *inside* a superconductor | Meissner: ∇²B = B/λ² | [5.3](./phase-5-superconductivity/module-5.3-ginzburg-landau-theory.md) |
| Quantizing gauge fields · 规范场量子化 | BRST (residual rigid) | Faddeev–Popov ghosts | s² = 0, ℒ_gf+gh = sΨ | [8.1 §E](./phase-8-particle-physics-and-standard-model/module-8.1-derivations.md), [6.5](./phase-6-quantum-field-theory/module-6.5-canonical-quantization.md) |
| When symmetry fails quantally · 量子反常 | anomalous currents | triangle anomaly | ∂_μj^{μ5} = (e²/16π²)ε^{μνρσ}F_{μν}F_{ρσ} | [6.9](./phase-6-quantum-field-theory/module-6.9-anomalies-and-nonperturbative-qft.md) |
| Synthesis · 综合 | SU(3)×SU(2)×U(1) | the Standard Model | one Lagrangian | [8.5](./phase-8-particle-physics-and-standard-model/module-8.5-standard-model-and-beyond.md) |
...
from point to point. Module 0.7 builds U(1) (phases e^{iα}), SU(2), SU(3), their generators
...
e^{iα}）、SU(2)、SU(3)，它们的生成元 T^a、李代数 [T^a, T^b] = if^{abc}T^c，以及将成为
...
Maxwell's four equations, the potential A_μ = (φ/c, **A**), the field tensor
F_{μν} = ∂_μA_ν − ∂_νA_μ, and the gauge freedom A_μ → A_μ + ∂_μχ are the prototype. The
derivations show: (i) F_{μν} packages **E** and **B** and is automatically gauge-invariant;
(ii) ∂_μF^{μν} = μ₀J^ν reproduces Gauss + Ampère–Maxwell; (iii) — **newly added** — all of
this follows from a *single action* S = ∫(−¼F_{μν}F^{μν}/μ₀ − J^μA_μ)d⁴x by varying A_μ,
with the homogeneous equations being the Bianchi identity of F = dA.

麦克斯韦四方程、势 A_μ = (φ/c, **A**)、场张量 F_{μν} = ∂_μA_ν − ∂_νA_μ，以及规范自由
A_μ → A_μ + ∂_μχ 是原型。推导给出：(i) F_{μν} 打包 **E** 与 **B** 且自动规范不变；
(ii) ∂_μF^{μν} = μ₀J^ν 重现高斯 + 安培–麦克斯韦；(iii)——**新增**——这一切都来自*单一作用量*
S = ∫(−¼F_{μν}F^{μν}/μ₀ − J^μA_μ)d⁴x 对 A_μ 的变分，齐次方程即 F = dA 的比安基恒等式。
...
A free Dirac field has a *global* U(1) symmetry ψ → e^{iα}ψ (Noether → conserved charge).
Promote α → α(x), and the kinetic term breaks — unless you introduce A_μ with the transform
A_μ → A_μ − (1/e)∂_μα and replace ∂_μ → D_μ = ∂_μ + ieA_μ. **The photon is the price of local
phase invariance.** This is *minimal coupling*; its quantum theory is QED.

自由狄拉克场有*整体* U(1) 对称性 ψ → e^{iα}ψ（诺特 → 守恒荷）。令 α → α(x)，动能项破坏
——除非引入 A_μ，其变换为 A_μ → A_μ − (1/e)∂_μα，并将 ∂_μ → D_μ = ∂_μ + ieA_μ。**光子是
...
D_μ = ∂_μ − igA^a_μT^a and the field strength acquire an extra, *self-interaction* term:
F^a_{μν} = ∂_μA^a_ν − ∂_νA^a_μ + gf^{abc}A^b_μA^c_ν. Because the gauge bosons carry the charge
...
用非阿贝尔群 G = SU(N) 重复此构造。协变导数 D_μ = ∂_μ − igA^a_μT^a 与场强获得一个额外的
*自相互作用*项：F^a_{μν} = ∂_μA^a_ν − ∂_νA^a_μ + gf^{abc}A^b_μA^c_ν。由于规范玻色子携带它们
...
Color SU(3), eight gluons, the running coupling, and asymptotic freedom (β₀ = 11 − 2n_f/3).
The gluon self-coupling of Stage 4 makes the β function negative — the coupling weakens at
...
色 SU(3)、八个胶子、跑动耦合与渐近自由（β₀ = 11 − 2n_f/3）。第 4 阶段的胶子自耦合使 β
函数为负——耦合在高能下减弱。这正是第 7 阶段的**法捷耶夫–波波夫鬼粒子**首次发挥作用之处：
它们的圈为 +11C_A/3 提供部分贡献。

→ [8.3 QCD](./phase-8-particle-physics-and-standard-model/module-8.3-quantum-chromodynamics.md) · [derivations (β function)](./phase-8-particle-physics-and-standard-model/module-8.3-derivations.md)

## 6. Spontaneously broken gauge symmetry: mass without breaking gauge invariance · 自发破缺的规范对称性

A gauge boson is massless because a mass term m²A_μA^μ is not gauge-invariant. The **Higgs
mechanism** evades this: a scalar field with a nonzero vacuum value breaks SU(2)_L × U(1)_Y
down to U(1)_em; the would-be Goldstone bosons become the longitudinal polarizations of W±
and Z⁰, which thereby gain mass, while the photon stays massless. Remarkably, the *same
physics* appears in condensed matter: inside a superconductor the photon is effectively
massive (Meissner effect, finite penetration depth λ) — the **Anderson–Higgs mechanism**,
described classically by Ginzburg–Landau theory.

规范玻色子无质量，因为质量项 m²A_μA^μ 非规范不变。**希格斯机制**规避了这一点：一个具有非零
真空值的标量场将 SU(2)_L × U(1)_Y 破缺至 U(1)_em；本应是戈德斯通玻色子的自由度变成 W± 和
Z⁰ 的纵向极化，由此获得质量，而光子保持无质量。值得注意的是，*相同的物理*出现在凝聚态中：
超导体内部光子有效地获得质量（迈斯纳效应、有限穿透深度 λ）——即**安德森–希格斯机制**，由
金兹堡–朗道理论在经典层面描述。
...
−(1/2ξ)(∂·A)² and the **Faddeev–Popov ghosts**. What survives is a single rigid fermionic
symmetry — **BRST symmetry**, nilpotent (s² = 0) — whose cohomology defines the physical
...
−(1/2ξ)(∂·A)² 与**法捷耶夫–波波夫鬼粒子**。幸存下来的是单一刚性费米型对称性——**BRST 对称
性**，幂零（s² = 0）——其上同调定义物理态、保证幺正性，并产生控制重整化的斯拉夫诺夫–泰勒
...
content is precisely arranged for the gauge anomalies to cancel (Σ charges = 0 per
generation). The surviving *global* anomaly (π⁰ → γγ) is a triumph, not a problem.

经典作用量的对称性可被量子化破坏（三角图）。*规范*反常将是致命的——它破坏幺正性——所以标准
模型的费米子内容被精确安排使规范反常相消（每代 Σ 荷 = 0）。幸存的*整体*反常（π⁰ → γγ）是
成就而非问题。
...
SU(3)_c × SU(2)_L × U(1)_Y, one gauge-covariant derivative acting on every field, three
...
SU(3)_c × SU(2)_L × U(1)_Y，一个作用于每个场的规范协变导数，三个朝向（近似）统一跑动的规范
...
|---|---|
| Covariant derivative (abelian) · 协变导数（阿贝尔） | D_μ = ∂_μ + ieA_μ |
| Covariant derivative (non-abelian) · 协变导数（非阿贝尔） | D_μ = ∂_μ − igA^a_μT^a |
| Field strength (abelian) · 场强（阿贝尔） | F_{μν} = ∂_μA_ν − ∂_νA_μ |
| Field strength (non-abelian) · 场强（非阿贝尔） | F^a_{μν} = ∂_μA^a_ν − ∂_νA^a_μ + gf^{abc}A^b_μA^c_ν |
| Yang–Mills action · 杨–米尔斯作用量 | S = −¼∫ F^a_{μν}F^{aμν} d⁴x |
| Equation of motion · 运动方程 | D_μF^{aμν} = J^{aν} |
| Bianchi identity · 比安基恒等式 | D_μF̃^{aμν} = 0 |
| Gauge-fixed Lagrangian · 规范固定拉格朗日量 | ℒ = −¼F² − (1/2ξ)(∂·A)² + c̄(−∂·D)c |
| BRST nilpotency · BRST 幂零性 | s² = 0,  ℒ_gf+gh = sΨ |
