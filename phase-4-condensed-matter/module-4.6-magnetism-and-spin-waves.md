...
**Definition.** Magnetism in solids arises from electronic spin and orbital angular momentum. In an external field $\mathbf{B}$, a material develops a magnetization $\mathbf{M} = \chi\mathbf{H}$ where $\chi$ is the magnetic susceptibility. **Diamagnets** ($\chi < 0$) weakly repel fields; **paramagnets** ($\chi > 0$) weakly attract them. Paramagnetism from localized magnetic moments obeys the **Curie law**:

$$ \chi = \frac{C}{T}, \qquad C = \frac{n \mu_0 \mu_{\mathrm{eff}}^2}{k_B} $$

where $n$ is the moment density and $\mu_{\mathrm{eff}}$ is the effective magnetic moment per site.

**定义。** 固体中的磁性来源于电子自旋和轨道角动量。在外场 $\mathbf{B}$ 中，材料产生磁化强度 $\mathbf{M} = \chi\mathbf{H}$，其中 $\chi$ 为磁化率。**抗磁体**（$\chi < 0$）微弱排斥磁场；**顺磁体**（$\chi > 0$）微弱吸引磁场。局域磁矩的顺磁性服从**居里定律**：

$$ \chi = \frac{C}{T}, \qquad C = \frac{n \mu_0 \mu_{\mathrm{eff}}^2}{k_B} $$

其中 $n$ 为磁矩密度，$\mu_{\mathrm{eff}}$ 为每个格点的有效磁矩。

**Demonstration.** The Curie law follows directly from the two-level partition function for a spin-$\tfrac12$ moment in a field (see Derivations A). For $S = \tfrac12$ in field $B$, the partition function $Z = 2\cosh(\mu_B B/k_B T)$ gives $M = n \mu_B \tanh(\mu_B B/k_B T)$. In the high-temperature (weak-field) limit $\mu_B B \ll k_B T$, $\tanh(x) \approx x$ and $\chi \propto 1/T$.

**演示。** 居里定律直接来源于自旋-$\tfrac12$ 磁矩在磁场中的两态配分函数（见推导 A）。对于 $S = \tfrac12$ 在磁场 $B$ 中，配分函数 $Z = 2\cosh(\mu_B B/k_B T)$ 给出 $M = n \mu_B \tanh(\mu_B B/k_B T)$。在高温（弱场）极限 $\mu_B B \ll k_B T$ 下，$\tanh(x) \approx x$，从而 $\chi \propto 1/T$。
...
where $J$ is the exchange constant and the sum is over nearest-neighbor pairs. $J > 0$ favors parallel spins $\rightarrow$ **ferromagnetism**; $J < 0$ favors antiparallel spins $\rightarrow$ **antiferromagnetism**. The simpler **Ising model** replaces the dot product with $S_i^z S_j^z$ (scalar spins).

**定义。** **交换相互作用**是将电子间库仑排斥与泡利不相容原理相结合的量子力学效应：两个处于重叠轨道的电子必须将其空间波函数反对称化，这有效地耦合了它们的自旋。由此给出**海森堡模型**：

$$ H = -J \sum_{\langle i,j\rangle} \mathbf{S}_i \cdot \mathbf{S}_j $$

其中 $J$ 为交换常数，求和遍历最近邻对。$J > 0$ 有利于自旋平行 $\rightarrow$ **铁磁性**；$J < 0$ 有利于自旋反平行 $\rightarrow$ **反铁磁性**。更简单的**伊辛模型**将点积替换为 $S_i^z S_j^z$（标量自旋）。

**Application — Mean-field theory & the Curie–Weiss law.** Replace the interactions on site $i$ by an effective molecular field proportional to the mean magnetization $m = \langle S^z\rangle$. Self-consistency gives a critical temperature
...
Below $T_c$ a spontaneous magnetization $m(T) > 0$ appears, vanishing near $T_c$ as $m \propto (T_c - T)^{1/2}$ (mean-field critical exponent $\beta = 1/2$). See Derivations B for the full mean-field calculation.

**应用——平均场理论与居里–外斯定律。** 将格点 $i$ 上的相互作用替换为正比于平均磁化强度 $m = \langle S^z\rangle$ 的有效分子场。自洽条件给出临界温度
...
$T_c$ 以下出现自发磁化 $m(T) > 0$，在 $T_c$ 附近趋向零时 $m \propto (T_c - T)^{1/2}$（平均场临界指数 $\beta = 1/2$）。完整平均场计算见推导 B。
...
$$ \hbar\omega_{\mathbf{k}} = 4 J S \left(3 - \cos k_x a - \cos k_y a - \cos k_z a\right). $$

For long wavelengths ($ka \ll 1$) this reduces to

$$ \hbar\omega_{\mathbf{k}} \approx 2 J S a^2 k^2 \quad \rightarrow \quad \omega \propto k^2. $$

This **quadratic dispersion** (contrasting with the linear acoustic-phonon dispersion $\omega \propto k$) is a hallmark of ferromagnetic magnons.
...
这种**二次型色散**（与线性声学声子色散 $\omega \propto k$ 形成对比）是铁磁磁振子的标志。

**Application — The Bloch $T^{3/2}$ law.** The magnon occupation follows the Bose–Einstein distribution. At low $T$, integrating over the magnon density of states (which scales as $\omega^{1/2}\,d\omega$ for a $\omega \propto k^2$ dispersion in 3D) gives the thermally excited magnon density $\propto T^{3/2}$. Each excited magnon flips one spin, reducing the magnetization:

$$ M(T) = M(0)\left[\,1 - A\,T^{3/2} + \dots\,\right] \qquad \text{(Bloch law)} $$

where $A$ is a material-dependent constant. See Derivations D for the full calculation.

**应用——布洛赫 $T^{3/2}$ 定律。** 磁振子占据数服从玻色–爱因斯坦分布。在低温下，对磁振子态密度积分（对三维空间 $\omega \propto k^2$ 色散，态密度 $\propto \omega^{1/2}\,d\omega$）给出热激发磁振子密度 $\propto T^{3/2}$。每个激发磁振子翻转一个自旋，从而降低磁化强度：

$$ M(T) = M(0)\left[\,1 - A\,T^{3/2} + \dots\,\right] \qquad \text{(布洛赫定律)} $$

其中 $A$ 为与材料有关的常数。完整计算见推导 D。

**Definition — Antiferromagnetism & Néel order.** When $J < 0$ the spins on the two sublattices (call them A and B) align antiparallel below the **Néel temperature** $T_N$. The **Néel state** has zero net magnetization but a staggered magnetization (the **order parameter**). Antiferromagnetic magnons have **linear** dispersion $\omega \propto k$ at long wavelengths — a key distinction from the ferromagnetic $\omega \propto k^2$ — and form two branches due to the two-sublattice structure.
