...
- Binding energy per nucleon peaks at $A\approx56$ (Fe/Ni) · 比结合能峰在铁
- Bethe–Weizsäcker SEMF (volume, surface, Coulomb, asymmetry, pairing) · 半经验质量公式
- Magic numbers from the nuclear spin–orbit force · 幻数
- $N(t)=N_0 e^{-\lambda t}$, $t_{1/2}=\ln2/\lambda$ · 放射性衰变
...
**1.** $B/A$ rises to a peak at $A\approx56$, then falls. Light-nucleus fusion and heavy-nucleus fission both move toward the peak, releasing energy. · 比结合能曲线;聚变与裂变均向峰移动放能。
**2.** $B=a_V A-a_S A^{2/3}-a_C\frac{Z(Z-1)}{A^{1/3}}-a_A\frac{(N-Z)^2}{A}\pm\delta$: volume (saturating strong force), surface (fewer bonds at the edge), Coulomb (proton repulsion), asymmetry (Pauli), pairing. · 五项及其来源。
**3.** Magic numbers 2, 8, 20, 28, 50, 82, 126. The spin–orbit term strongly lowers the $j=l+\tfrac12$ level; for $1f_{7/2}$ it drops below the rest, opening the gap at $N=28$. · 自旋–轨道力给出 $N=28$ 间隙。
**4.** Memoryless decay $dN=-\lambda N\,dt\Rightarrow N(t)=N_0 e^{-\lambda t}$; $t_{1/2}=\ln2/\lambda$. · 衰变律与半衰期。
