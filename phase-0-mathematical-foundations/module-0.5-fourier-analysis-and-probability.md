# Module 0.5 — Fourier Analysis & Probability ⭐ · 傅里叶分析与概率
...
傅里叶分析将复杂性分解为各频率成分；概率则对不确定性进行量化。在量子力学中，这两种思想交汇——位置与动量之间的傅里叶关系*就是*不确定性原理，而概率幅度*正是*被分解的那些量。
...
**定义。** 任何周期为 L 的周期函数 f(x) 都可写成**傅里叶级数**——正弦与余弦（或复指数）的叠加：

  f(x) = Σ_{n=−∞}^{∞} cₙ e^{i 2πnx/L},   cₙ = (1/L) ∫_0^L f(x) e^{−i 2πnx/L} dx。

对整条实数轴上的非周期函数，离散求和变为连续积分——**傅里叶变换**及其逆变换：

  f̃(k) = ∫_{−∞}^{∞} f(x) e^{−ikx} dx,   f(x) = (1/2π) ∫_{−∞}^{∞} f̃(k) e^{ikx} dk。

**狄拉克 δ 函数** δ(x) 是该变换的单位元：δ̃(k) = 1，意味着它等量地包含所有频率。它满足 ∫ δ(x−a) f(x) dx = f(a)，可表示为 δ(x) = (1/2π) ∫ eⁱᵏˣ dk。**卷积定理**指出 (f * g)~ = f̃ · g̃：实空间中的卷积对应频率空间中的逐点相乘，反之亦然。
...
当 σ 大（x 空间宽）时，f̃ 在 k 空间窄。当 σ 小（x 空间窄）时，f̃ 在 k 空间宽。定量地，Δx · Δk ≥ 1/2。这一**倒数展宽**是傅里叶变换对的数学恒等式——与测量扰动无关。其物理诠释将在模块 3.1 中给出。
...
**应用。** 在量子力学中，动量空间波函数 ψ̃(p) 恰好是位置空间波函数 ψ(x) 的傅里叶变换，其中 k = p/ℏ（模块 3.1）。上述高斯演示即海森堡不确定性原理 ΔxΔp ≥ ℏ/2。在凝聚态物理中，**倒易格子**是晶体布拉维格子的傅里叶变换，布拉格衍射是对电荷密度的傅里叶分析（模块 4.2）。卷积定理支撑着探测器与光源如何展宽谱线的理解。
...
它是自身的傅里叶变换（相差一个缩放因子），归一化为 1，并在固定均值和方差的条件下使熵最大。**中心极限定理**指出，许多独立随机变量之和——无论各自分布如何——在分布上收敛到高斯分布。
...
**应用。** 在量子力学中，|ψ(x)|² 是概率密度：位置的期望值为 ⟨x⟩ = ∫ x|ψ|² dx，不确定度 Δx = √(⟨x²⟩ − ⟨x⟩²) 恰好是对 |ψ|² 所求的标准差（模块 3.1）。中心极限定理解释了为何高斯分布出现在热噪声、扩散以及谐振子基态波函数中。**态密度**——在能量 E 附近找到能级的概率密度——直接进入费米黄金规则（模块 3.8）以及统计力学中的费米–狄拉克分布和玻色–爱因斯坦分布。
...
## Module 0.5 Self-Test (blank page) · 模块 0.5 自测（空白页）
...
1. 写出傅里叶变换对，并叙述卷积定理。
2. 证明高斯函数的傅里叶变换仍是高斯函数，并解释倒数展宽结果。
3. 为连续分布定义 ⟨x⟩、⟨x²⟩ 和 σ²；对 [0,∞) 上的 p(x) = e^{−x} 计算这些量。
4. 用一句话解释高斯函数的傅里叶变换结果如何成为海森堡不确定性原理。
5. 叙述中心极限定理，并给出两个物理情境说明其解释高斯分布出现的原因。
