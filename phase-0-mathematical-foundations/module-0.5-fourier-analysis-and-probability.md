...
$$ f(x) = \sum_{n=-\infty}^{\infty} c_n\, e^{i 2\pi n x/L}, \qquad c_n = \frac{1}{L} \int_0^L f(x)\, e^{-i 2\pi n x/L}\, dx. $$
...
The **Dirac delta** $\delta(x)$ is the identity element of this transform: $\tilde{\delta}(k) = 1$, meaning it contains all frequencies equally. It satisfies $\int \delta(x-a) f(x)\, dx = f(a)$ and can be represented as $\delta(x) = (1/2\pi) \int e^{ikx}\, dk$. The **convolution theorem** states that $\widetilde{(f * g)} = \tilde{f} \cdot \tilde{g}$: convolution in real space is pointwise multiplication in frequency space, and vice versa.

**狄拉克 delta 函数** $\delta(x)$ 是该变换的恒等元素：$\tilde{\delta}(k) = 1$，意味着它均匀地包含所有频率。它满足 $\int \delta(x-a) f(x)\, dx = f(a)$，可以表示为 $\delta(x) = (1/2\pi) \int e^{ikx}\, dk$。**卷积定理**指出 $\widetilde{(f * g)} = \tilde{f} \cdot \tilde{g}$：实空间中的卷积对应频率空间中的逐点相乘，反之亦然。

**Demonstration.** The Fourier transform of a **Gaussian** $f(x) = e^{-x^2/2\sigma^2}$ is another Gaussian:

**演示。** **高斯函数** $f(x) = e^{-x^2/2\sigma^2}$ 的傅里叶变换仍是高斯函数：

$$ \tilde{f}(k) = \sigma\sqrt{2\pi}\, e^{-\sigma^2 k^2/2}. $$

When $\sigma$ is large (broad in $x$), $\tilde{f}$ is narrow in $k$. When $\sigma$ is small (narrow in $x$), $\tilde{f}$ is wide in $k$. Quantitatively, $\Delta x \cdot \Delta k \ge 1/2$. This **reciprocal spread** is a mathematical identity of Fourier pairs — it has nothing to do with measurement disturbance yet. Its physical interpretation arrives in Module 3.1.

当 $\sigma$ 较大（$x$ 空间中较宽）时，$\tilde{f}$ 在 $k$ 空间中较窄。当 $\sigma$ 较小（$x$ 空间中较窄）时，$\tilde{f}$ 在 $k$ 空间中较宽。定量地，$\Delta x \cdot \Delta k \ge 1/2$。这一**互反展宽**是傅里叶变换对的数学恒等式——与测量扰动无关。其物理解释将在模块 3.1 中给出。

**Application.** In quantum mechanics, the momentum-space wavefunction $\tilde{\psi}(p)$ is exactly the Fourier transform of the position-space wavefunction $\psi(x)$, with $k = p/\hbar$ (Module 3.1). The Gaussian demonstration above is then the Heisenberg uncertainty principle $\Delta x \Delta p \ge \hbar/2$. In condensed matter, the **reciprocal lattice** is the Fourier transform of the crystal's Bravais lattice, and Bragg diffraction is Fourier analysis of the charge density (Module 4.2). The convolution theorem underpins how detectors and sources broaden spectral lines.

**应用。** 在量子力学中，动量空间波函数 $\tilde{\psi}(p)$ 恰好是位置空间波函数 $\psi(x)$ 的傅里叶变换，其中 $k = p/\hbar$（模块 3.1）。上述高斯函数演示由此成为海森堡不确定性原理 $\Delta x \Delta p \ge \hbar/2$。在凝聚态物理中，**倒格子**是晶体布拉维格子的傅里叶变换，布拉格衍射是对电荷密度进行傅里叶分析（模块 4.2）。卷积定理是探测器和光源展宽谱线的基本原理。

---

## 2. Probability · 概率

**Definition.** A **random variable** $X$ has a **probability density** $p(x)$ (with $p(x) \ge 0$ and $\int p(x)\, dx = 1$). The **mean** (expectation value) and **variance** are:

**定义。** **随机变量** $X$ 具有**概率密度** $p(x)$（满足 $p(x) \ge 0$ 且 $\int p(x)\, dx = 1$）。**均值**（期望值）和**方差**为：

$$ \langle x\rangle = \int x\, p(x)\, dx, \qquad \sigma^2 = \langle (x - \langle x\rangle)^2\rangle = \langle x^2\rangle - \langle x\rangle^2. $$

The standard deviation $\sigma = \sqrt{\langle x^2\rangle - \langle x\rangle^2}$ measures the spread. The **Gaussian (normal) distribution** with mean $\mu$ and standard deviation $\sigma$ is

标准差 $\sigma = \sqrt{\langle x^2\rangle - \langle x\rangle^2}$ 衡量分布的展宽。均值为 $\mu$、标准差为 $\sigma$ 的**高斯（正态）分布**为
...
**Application.** In quantum mechanics, $|\psi(x)|^2$ is a probability density: the expectation value of position is $\langle x\rangle = \int x|\psi|^2\, dx$ and the uncertainty $\Delta x = \sqrt{\langle x^2\rangle - \langle x\rangle^2}$ is exactly the standard deviation applied to $|\psi|^2$ (Module 3.1). The central limit theorem explains why the Gaussian appears in thermal noise, diffusion, and the ground-state wavefunction of the harmonic oscillator. The **density of states** — the probability density of finding an energy level near $E$ — feeds directly into Fermi's golden rule for transition rates (Module 3.8) and into the Fermi–Dirac and Bose–Einstein distributions in statistical mechanics.
...
3. Define $\langle x\rangle$, $\langle x^2\rangle$, and $\sigma^2$ for a continuous distribution; compute them for $p(x) = e^{-x}$ on $[0,\infty)$.
...
3. 为连续分布定义 $\langle x\rangle$、$\langle x^2\rangle$ 和 $\sigma^2$；对 $[0,\infty)$ 上的 $p(x) = e^{-x}$ 计算它们。
