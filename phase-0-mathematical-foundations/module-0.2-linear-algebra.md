...
**定义。** **向量空间**是一组对象（向量）的集合，其元素可以相加和数乘且结果仍在集合内。**内积** ⟨u|v⟩ 为每对向量指定一个标量，满足线性性、共轭对称性（⟨u|v⟩ = ⟨v|u⟩*）以及正定性（⟨v|v⟩ ≥ 0）。**范数**为 ‖v‖ = √⟨v|v⟩。若向量组 {eᵢ} 满足 ⟨eᵢ|eⱼ⟩ = δᵢⱼ，则称其为**正交归一**的；若每个向量都可写成 v = Σᵢ ⟨eᵢ|v⟩ eᵢ，则称其为**完备**的（即基）。系数 ⟨eᵢ|v⟩ 为该基下的**分量**。
...
**演示。** 在 ℝ² 中，标准基 {(1,0), (0,1)} 在点积意义下是正交归一的。旋转到一个新的正交归一基会改变分量，但不改变底层向量本身——这一道理延伸到量子力学中：能量本征态基与位置本征态基以不同方式表示同一个态。
...
**应用。** 量子态的希尔伯特空间是一个（通常为无穷维的）内积空间。内积 ⟨ψ|φ⟩ 给出跃迁振幅；能量本征态基的完备性意味着任何态都可以写成叠加——这是所有波动力学的基础（模块 3.3）。
...
其中 v ≠ 0 为**本征向量**，λ 为对应的**本征值**。两类特殊算符在物理中占主导地位：
...
- **厄米算符**（A = A†，其中 A†ᵢⱼ = Aⱼᵢ*）：本征值为**实数**，不同本征值对应的本征向量**正交**，本征向量构成完备基——即**谱定理**。
- **幺正算符**（U†U = I）：保持范数不变（‖Uv‖ = ‖v‖），本征值的模为 1。

The three **Pauli matrices** are the foundational Hermitian, traceless, unitary-up-to-phase operators of spin-½:

三个**泡利矩阵**是自旋-½ 系统的基本厄米、无迹、相位意义下幺正的算符：
...
它们满足 σᵢ σⱼ = δᵢⱼ I + i εᵢⱼₖ σₖ，是所有两能级量子系统的构建模块。
...
对 λ = +1：(σ_x − I)v = 0 给出 v₁ = v₂，故本征向量为 (1/√2)(1, 1)ᵀ。
对 λ = −1：v₁ = −v₂，给出 (1/√2)(1, −1)ᵀ。
这两个正交归一本征向量就是 **|+x⟩, |−x⟩ 基**——沿 x 方向的自旋本征态。
