...
**定义。** **机械波**是在介质中传播、输运能量和动量而不净输运物质的扰动。小扰动满足**波动方程** ∂²u/∂t² = c²∂²u/∂x²（三维为 ∂²u/∂t² = c²∇²u），这是一个用分离变量法（模块 0.3）求解的偏微分方程。其一维通解是**达朗贝尔形式** u(x,t) = f(x − ct) + g(x + ct)：以速度 c 向右和向左传播的任意波形。
...
**演示。** 张力为 T、线密度为 μ 的弦支持横波，**c = √(T/μ)**。质量和弹簧的连续链（模块 1.6 耦合振子的连续极限）精确重现这一方程——波是无穷多振子的集体简正模式。谐波解 u = A cos(kx − ωt) 给出**色散关系** ω = ck，波长 λ = 2π/k，频率 f = ω/2π。
...
**应用。** 简正模式的任意叠加本身就是解；将扰动分解为谐波分量是**傅里叶分析**（模块 0.5）。有界弦上的驻波量子化为谐波 fₙ = n c/2L——这与无限方势阱（模块 3.2）中量子化能量的边值逻辑相同。
...
**定义。** **声音**是流体中的纵向压力波，速度为 **c = √(B/ρ)**（B = 体积模量，ρ = 密度）。波携带能量通量（强度 ∝ 振幅²），并表现出**反射、折射、干涉和衍射**；运动的声源或观察者产生**多普勒频移** f′ = f (c ± v_obs)/(c ∓ v_src)。
...
**演示。** 频率相近的两列波叠加产生频率为 |f₁ − f₂| 的**拍**；波遇到边界时部分反射，在固定端有相位翻转。群速度 v_g = dω/dk 决定波*包*（及其能量）在色散介质中实际传播的方式。
