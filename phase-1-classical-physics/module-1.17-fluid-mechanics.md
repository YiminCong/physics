...
**定义。** **流体**是由场描述的连续介质：密度 ρ(r,t)、压强 P(r,t) 和速度 **v**(r,t)。质量守恒是**连续性方程** ∂ρ/∂t + ∇·(ρ**v**) = 0。对于理想（无粘）流体，单位体积的牛顿第二定律是**欧拉方程** ρ(∂**v**/∂t + (**v**·∇)**v**) = −∇P + ρ**g**。
...
**演示。** 对于定常、不可压缩、无旋流动，沿流线积分欧拉方程给出**伯努利定理** P + ½ρv² + ρgh = 常数——流速越快压强越低。这解释了机翼的升力、文丘里管中的升力，以及为什么浴帘会向内鼓起。
...
**应用。** 连续性方程是场形式的原型**守恒定律**（模块 1.2），在电荷（模块 1.10）和概率（模块 3.1）中重复使用。非线性对流项 (**v**·∇)**v** 是湍流（见下）的种子，也是物理学中许多困难的根源。
...
**定义。** 真实流体通过**粘度** η 抵抗剪切。在欧拉方程中加入粘性力 η∇²**v**，得到**纳维–斯托克斯方程** ρ(∂**v**/∂t + (**v**·∇)**v**) = −∇P + η∇²**v** + ρ**g**——流体动力学的主方程。无量纲的**雷诺数** Re = ρvL/η 衡量惯性力与粘性力之比。
...
**演示。** 在低雷诺数（≪1）时粘性力主导：平滑的**层流**，例如管内具有抛物线速度剖面的泊肃叶流。在高雷诺数时非线性项主导，流动变为**湍流**——混沌、涡旋，细节实际上不可预测（模块 1.19 混沌的表现）。**涡度** ω = ∇×**v** 量化局部旋转。
...
3. 雷诺数比较什么？估算水以 1 m/s 流过直径 1 cm 管道时的 Re（η/ρ ≈ 10⁻⁶ m²/s）。
