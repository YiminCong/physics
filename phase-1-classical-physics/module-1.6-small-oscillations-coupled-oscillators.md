...
- $m\ddot x + b\dot x + kx = F_0\cos(\omega t)$ — damped, driven oscillator; resonance at $\omega \approx \omega_0$ · 阻尼受迫振子与共振
- $K\mathbf{A} = \omega^2 M\mathbf{A}$ — normal modes as a generalized eigenproblem · 简正模式即广义本征问题
- Any small-oscillation system decouples into independent normal modes · 任意小振动系统解耦为独立简正模
- This is the seed of phonons (4.3) and quantum fields (6.1) · 通往声子与量子场
...
**1.** Steady state $x(t)=A\cos(\omega t-\delta)$ with $A=\dfrac{F_0/m}{\sqrt{(\omega_0^2-\omega^2)^2+(\gamma\omega)^2}}$; the amplitude peaks at $\omega=\sqrt{\omega_0^2-\gamma^2/2}\approx\omega_0$ — resonance. · 稳态振幅在 $\omega\approx\omega_0$ 最大(共振)。

**2.** $Q=\omega_0/\gamma$: a high $Q$ means a sharp resonance peak and slow energy decay (the oscillator rings for $\sim Q/2\pi$ cycles). · $Q=\omega_0/\gamma$,高 $Q$ 共振尖锐、衰减慢。

**3.** Diagonalizing the symmetric two-mass system gives $\omega_1=\sqrt{k/m}$ (masses in phase, spring between them unstretched) and $\omega_2=\sqrt{3k/m}$ (anti-phase). · 简正频率 $\sqrt{k/m}$(同相)与 $\sqrt{3k/m}$(反相)。

**4.** The equations of motion become $K\mathbf A=\omega^2 M\mathbf A$, a generalized eigenvalue problem; the stiffness matrix $K$ (with mass matrix $M$) is diagonalized, its eigenvalues giving $\omega^2$ and eigenvectors the normal modes. · 化为广义本征问题 $K\mathbf A=\omega^2 M\mathbf A$。
